/**
 * 알밤yammkki 채널 쇼츠 수집 - 설명 또는 댓글에 주소가 있는 쇼츠만 추가
 *
 * 실행: npm run fetch-yammkki
 */
...
const SHORTS_URL = 'https://www.youtube.com/@%EC%96%8C%EB%81%BCyammkki/shorts';
const CHANNEL = {
  id: 3,
  name: '알밤yammkki',
  subs: '조회수 확인 필요',
  ...
  color: '#4CAF50',
  youtubeChannelId: '',
  url: 'https://www.youtube.com/@%EC%96%8C%EB%81%BCyammkki',
};
...
async function main() {
  console.log('알밤yammkki 쇼츠 수집 - 설명/댓글에 주소 있는 것만 추가\n');
  ...
  let channel = db.channels.find((c) => c.id === CHANNEL.id || c.url?.includes('yammkki'));
  if (!channel) {
    db.channels.push({
      id: CHANNEL.id,
      name: CHANNEL.name,
  ...
  for (let i = 0; i < unique.length; i++) {
    ...
    try {
      ...
      console.warn(`  [${i + 1}] ${s.title?.slice(0, 40)}... - 조회 실패`);
      await sleep(800);
      continue;
    }
    ...
    if (!extracted) {
      console.log(`  [${i + 1}/${unique.length}] ${s.title?.slice(0, 45)}... - 주소 없음 (건너뜀)`);
      skipped++;
      await sleep(800);
      continue;
    }
